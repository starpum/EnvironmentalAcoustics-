...
def load_nor140(path, w):
    ...
    acc = data.iloc[:, 7:33].astype(float).to_numpy()  # 26 bands up to 2 kHz
    vel = (acc / w) * 1000 #conversion to mm
    disp = vel / w
...
def main():  
    ...
    for n, fname in enumerate(xls_files):
        ...
        fmt, acc, disp, vel = load_nor140(path, w)

        for i in range(len(fmt)):

            pt = f"Pump {n} - location {i+1}"
            ...
            plt.semilogx(freq, acc[i], marker='o', label='Acceleration [m/s-1]')
            ...
            # Displacement plot
            plt.figure(figsize=(8, 6))
            plt.semilogx(freq, disp[i], marker='s', label='Displacement [mm]')
            plt.title(f'{pt} - {fmt.iloc[i]}')
            plt.xlabel('Frequency (Hz)')
            plt.ylabel('Amplitude')
            plt.legend()
